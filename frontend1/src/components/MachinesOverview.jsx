  ...
  return (
    <div className="bg-gray-200 p-9 h-[100vh]">
      ...
      <div className="mb-4 flex justify-end">
        <DropdownButton text="Division" type="division" onSelect={handleFilterChange} />
        <DropdownButton text="Site" type="site" onSelect={handleFilterChange} />
        <DropdownButton text="Status" type="status" onSelect={handleFilterChange} />
      </div>
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="bg-black text-white">
              <th className="py-2 px-4 text-left">Machine ID</th>
              ...
              <th className="py-2 px-4 text-left">Grinding Date</th>
          ...
          <tbody>
            {filteredMachines.map(machine => (
              <tr key={machine.id} className="hover:bg-gray-100">
                ...
                <td className="py-2 px-4">{machine.grindingDate}</td>
                <td className="py-2 px-4">
                  <span className={machine.status === "Done" ? "text-green-500" : "text-red-500"}>
                    {machine.status}
                  </span>
                </td>
              </tr>
